...
A **Collection** input takes a whole collection of curve objects instead, lofted before the numbered inputs. **They are sorted by name**, so a numbered naming drives the order — `Curve.001`, `Curve.002`, `Curve.003`, or `Curve_001`, `Curve_002`, `Curve_003`. The sort is natural, so `Curve.2` comes before `Curve.10`. Nested collections are included, objects that are not curves are ignored, and each object's transform is applied.
...
| View Layer Overrides: per-layer output cannot produce a video file | A video stream cannot be split per layer, so the override is ignored for movie formats. **Multi-Layer Deep EXR does follow the override**: each layer gets its own single-part deep file at its own path, provided that path ends in `.exr` — a layer pointed at a format that cannot carry deep (PNG, JPEG…) writes flat and says so in the report. The standard **Deep EXR** output is unaffected and keeps its own `Deep/<layer>/` naming. |
